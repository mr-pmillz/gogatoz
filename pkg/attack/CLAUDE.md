...
13. **`--deconflict force` reuses the existing branch** -- it updates files in place and must never delete a protected or default branch. The resulting file commit triggers the pipeline to inspect.
...
17. **Payload YAML `- |` block scalar indentation** -- all `Generate*YAML()` functions that use `- |\n%s` must indent the script content with `indentBlock(strings.TrimSpace(script), 6)`. Unindented content causes GitLab to silently reject the YAML (pipeline created with 0 jobs, no yaml_errors field set). The `memorydump.go` and `branchmutator.go` generators already do this correctly -- follow their pattern.
18. **`_FUNCNAME || true` pattern** -- payload scripts that define a shell function and call it at the end (`_MEMDUMP`, `_ESCAPE`, `_C2CHANNEL`) must append `|| true` to the function invocation to prevent CI job failure when any internal command returns non-zero.
19. **`renderPayload()` split** -- the CLI switch for `--payload` is split across `renderPayload()` (original payloads) and `renderExpansionPayload()` (expansion track payloads) in `cmd/attack_helpers.go`. New payload types go in `renderExpansionPayload()` to keep cognitive complexity under the gocognit 30 threshold.
