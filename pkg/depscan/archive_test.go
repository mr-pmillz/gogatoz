...
func makeTarGz(t *testing.T, entries []tarEntry) []byte {
	...
	for _, entry := range entries {
		...
		hdr := &tar.Header{Typeflag: typeflag, PAXRecords: entry.pax}
		if typeflag == tar.TypeXGlobalHeader {
			// archive/tar requires global headers to contain PAXRecords only.
		} else {
			hdr.Name = entry.name
			hdr.Mode = 0o644
...
func TestExtractTarGz_AcceptsGitLabPAXGlobalMetadata(t *testing.T) {
	dest := t.TempDir()
	archive := makeTarGz(t, []tarEntry{
		{
			name:     "pax_global_header",
			typeflag: tar.TypeXGlobalHeader,
			pax:      map[string]string{"comment": "Generated by GitLab"},
		},
		{
			name: "release-metadata-service-deadbeef/bom.cdx.json",
			body: `{"bomFormat":"CycloneDX"}`,
		},
	})

	if err := extractTarGz(bytes.NewReader(archive), dest, ArchiveLimits{}); err != nil {
		t.Fatalf("extractTarGz with GitLab PAX metadata: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dest, "release-metadata-service-deadbeef", "bom.cdx.json")); err != nil {
		t.Fatalf("expected regular file after PAX metadata: %v", err)
	}
}
