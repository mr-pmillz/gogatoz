...
var findingCodeRegistry = map[string]FindingCodeInfo{
	...
	PackageExecutionTriggerID: {
		ID:          PackageExecutionTriggerID,
		Severity:    SeverityHigh,
		Title:       "Package contains an automatic execution trigger",
		Description: "A package contains an install, native-build, import-time, or interpreter-startup path that can execute without an explicit application action.",
		Remediation: "Inspect the referenced hook or entry point without executing it; remove undocumented automatic execution and rebuild from reviewed source.",
	},
	PackagePersistenceID: {
		ID:          PackagePersistenceID,
		Severity:    SeverityHigh,
		Title:       "Package contains a persistence indicator",
		Description: "A package contains a detached-process, developer-tool, cron, or systemd persistence path.",
		Remediation: "Quarantine the artifact, compare it with reviewed source, and remove undocumented developer-tool or operating-system persistence files.",
	},
	PackageExecutableID: {
		ID:          PackageExecutableID,
		Severity:    SeverityHigh,
		Title:       "Package contains an executable payload",
		Description: "A package member is executable, has native executable magic, or disguises executable bytes behind an inconsistent extension.",
		Remediation: "Verify the executable against reviewed source, platform expectations, and a cryptographic digest; remove disguised or undocumented binaries.",
	},
	PackageObfuscationID: {
		ID:          PackageObfuscationID,
		Severity:    SeverityMedium,
		Title:       "Package contains obfuscated code",
		Description: "Package code reconstructs security-sensitive strings from numeric byte arrays or another static obfuscation pattern.",
		Remediation: "Decode and review the hidden value in isolation, then require a transparent source change and reproducible artifact before release.",
	},
	ArtifactSourceDivergenceID: {
		ID:          ArtifactSourceDivergenceID,
		Severity:    SeverityHigh,
		Title:       "Published artifact diverges from reviewed source",
		Description: "The package contains files absent from reviewed source or different bytes at a matching source path.",
		Remediation: "Stop release consumption, identify the build step that introduced the divergence, and publish only from reviewed source using a reproducible, attestable build.",
	},
	ArtifactPartialBuildID: {
		ID:          ArtifactPartialBuildID,
		Severity:    SeverityMedium,
		Title:       "Published artifact appears to be a partial build",
		Description: "The artifact has substantially fewer files or bytes than reviewed source, indicating an incomplete or selectively published build.",
		Remediation: "Confirm the intended package contents and build include/exclude rules, compare with the prior release, and rebuild from the reviewed release commit.",
	},
	ProvenanceMismatchID: {
		ID:          ProvenanceMismatchID,
		Severity:    SeverityHigh,
		Title:       "Package provenance does not match release expectations",
		Description: "The attestation is missing or disagrees with an expected repository, commit, ref, or pipeline.",
		Remediation: "Reject the artifact and regenerate provenance from the reviewed release commit in the approved publishing pipeline.",
	},
	ReleaseTagMismatchID: {
		ID:          ReleaseTagMismatchID,
		Severity:    SeverityHigh,
		Title:       "Package version does not match its release tag",
		Description: "The version declared inside the package archive differs from the expected Git release tag.",
		Remediation: "Verify the release tag target and package manifest version, then rebuild and republish from the reviewed tag without mutating it.",
	},
	...
	"INCLUDE_PROJECT_UNPINNED": {
		...
		Remediation: "Pin project includes to a reviewed full commit SHA. Tags and branches can be moved after review and do not provide an immutable trust boundary. See: https://docs.gitlab.com/ee/ci/yaml/includes.html#syntax-for-include",
	},
	"INCLUDE_COMPONENT": {
		...
		Remediation: "Use trusted components pinned to a reviewed full commit SHA; tags and version selectors remain mutable. Review component inputs for injection risks. See: https://docs.gitlab.com/ee/ci/components/",
	},
	"SELF_HOSTED_EXPOSED": {
	...
	IncludeForbiddenVersionID: {
		...
		Description: "Project include is pinned to a branch name instead of a full commit SHA. Branch refs are mutable and allow the upstream project to change included code without notice.",
		Remediation: "Resolve the reviewed branch revision to a known full commit SHA and pin the include to that SHA. Tags are also mutable and are not a substitute for a commit pin. See: https://docs.gitlab.com/ee/ci/yaml/includes.html#syntax-for-include",
	},
	IncludeMutableRefID: {
		ID:          IncludeMutableRefID,
		Severity:    SeverityHigh,
		Title:       "Include uses mutable release ref",
		Description: "A project or component include uses a tag, custom branch, shortened SHA, or dynamic ref that can be moved to different code after review.",
		Remediation: "Resolve the reviewed version to a known full commit SHA and pin the include to that SHA. Monitor upstream advisories and update the pin only through reviewed changes. See: https://docs.gitlab.com/ee/ci/yaml/includes.html#syntax-for-include",
	},
