...
- **`cmd/`** — Cobra CLI commands (root, search, enumerate, attack, pivot, parse, api, report, notify, bloodhound, validate). Global flags and Viper config binding in `root.go`. Shared pterm UI helpers in `ui.go`. The `parse` and `bloodhound` commands override `PersistentPreRunE` to skip token/config init. The `validate` command probes a GitLab token's effective scopes and capabilities.
...
- **`pkg/analyze/`** — Vulnerability analysis engine. Multi-pass rule evaluation covering 34+ finding IDs: include risks, runner exposure, MR-triggered jobs on self-hosted runners, variable injection, artifact poisoning, plaintext secrets, fork MR risks, workflow broad rules, supply chain (script injection, self-merge, cache poisoning), dispatch/TOCTOU, Pwn Request deployments, AI prompt injection, LOTP tool execution (`LOTP_TOOL_EXEC`), cache key injection (`CACHE_KEY_INJECTION`), GitLab OIDC token exposure (`OIDC_TOKEN_MR_RISK`), downstream trigger chain abuse (`TRIGGER_CHAIN_RISK`), artifact report injection (`ARTIFACT_REPORT_INJECTION`), service command injection (`SERVICE_COMMAND_INJECTION`), remote include cache poisoning (`INCLUDE_REMOTE_CACHED`), workflow variable injection (`WORKFLOW_VAR_INJECTION`), spec:inputs injection risk (`SPEC_INPUTS_INJECTION_RISK`), trigger artifact risk (`TRIGGER_ARTIFACT_RISK`), rules security bypass (`RULES_SECURITY_BYPASS`), and cross-project needs risk (`NEEDS_PROJECT_RISK`). All script analysis uses `effectiveScripts()` to cover before_script/after_script phases. LOTP catalog in `lotp.go` covers 60+ tools from https://boostsecurityio.github.io/lotp/
- **`pkg/validate/`** — Token scope probing. `ProbeToken()` hits `/personal_access_tokens/self`, `/user`, and capability endpoints (projects, groups, users, runners, admin settings) to map effective permissions. Output: `TokenProfile` with identity, scopes, expiry, admin status, and per-endpoint capability results.
- **`pkg/enumerate/`** — Concurrent project scanner using a worker pool pattern. Expands groups to projects, resolves CI configs, runs analyzer, discovers runners and protected branches. Streams results via callback. `--self` discovers all projects the token owner is a member of; `--all-projects` discovers every project visible to the token on the instance. `--local <path>` scans local directories for `.gitlab-ci.yml` files without any API calls. Run-log runner detection (`logrunner.go`) extracts runner metadata from job traces as a fallback when `--runners` is not set. Post-enumeration attack surface recommender (`report/recommend.go`) synthesizes findings into actionable attack recommendations with suggested gogatoz commands.
...
- **`pkg/store/`** — SQLite-backed persistence for MCP scan results using GORM. Models: `ScanSession`, `SearchResult`, `EnumerateResult`, `Finding`, `AttackResult`, `AttackExfilSecret`, `SecretScanResult`, `SecretFinding`, `PivotSession`, `HarvestedCredential`, `ExfiltratedSecret`, `GraphNode`, `GraphEdge`, `KeyPair` (RSA keypair storage for `--auto-encrypt`). Auto-migrates on open, WAL mode enabled.
...
Enumerate → takes project IDs/paths from file/stdin/JSONL → expands groups (or discovers via `--self`/`--all-projects`) → fetches repo trees → parses `.gitlab-ci.yml` with include resolution → runs analyzer → streams findings.
...
- **Token-skip patterns for commands**: Commands that don't need a token override `PersistentPreRunE` (e.g., `parse`, `bloodhound`). The `enumerate --local` mode checks `enumLocalPath` before the token guard. New tokenless commands should follow the `parse` pattern (no-op PersistentPreRunE).
...
- **Attack command modes**: `cmd/attack.go` requires exactly one mode flag (`--commit-ci`, `--secrets`, `--cleanup`, `--deploy-key`, `--add-member`, `--ai-inject`, `--inject-script`, `--auto-merge`, `--harvest`, `--tamper-release`, `--tamper-package`, `--tamper-tag`) unless using `--discover-tags` or `--payload-only`. Payload types (original): `ror-shell`, `pwn-request`, `ror`, `secrets`, `git-hook`, `cache-poison`, `infostealer`, `memory-dump`, `supplychain-worm`, `container-escape`, `variable-inject`, `c2-channels`, `npm-tamper`, `vault-enum`, `k8s-secrets`, `dead-mans-switch`, `branch-mutator`, `sigstore`, `workflow-exfil`, `commit-prefix`, `release-tamper-pipeline`, `dep-confusion`, `runner-var-dump`, `nested-runner`. Expansion payloads (in `renderExpansionPayload()`): `remote-include-cache`, `workflow-vars`, `spec-inputs`, `rules-bypass`, `interruptible`, `oidc-federation`, `artifact-reports`, `image-poison`, `parallel-matrix`, `pre-get-sources`, `cache-key-poison`, `trigger-artifact`, `needs-project`. `--auto-encrypt` auto-generates RSA-4096 keypair for `--secrets` mode (keys stored in DB via `pkg/store.KeyPair`). Anti-forensics: `--cleanup-pipeline`, `--cleanup-jobs`. New attack modes require: adding a flag var, updating mode count validation, adding a handler block, and registering the flag in `init()`. New payload types go in `renderExpansionPayload()` in `cmd/attack_helpers.go`.
- **Enumerate discovery modes**: `--self` (all projects token owner is a member of via `membership=true`), `--all-projects` (every project visible to the token on the instance), `--local <path>` (offline scan of local `.gitlab-ci.yml` files, no token required). `--self` and `--all-projects` feed into the existing dedup + worker pool.
- **Validate command**: `gogatoz validate` probes a token's effective capabilities via `/personal_access_tokens/self`, `/user`, and 5 capability endpoints. No subcommands.
- **Attack surface recommender**: Enumerate text output includes an "Attack Surface Recommendations" section synthesized from aggregate findings. 6 categories (CI Injection, Pwn Request, Secrets Exfil, Lateral Movement, Supply Chain, Persistence) with suggested gogatoz commands. Built in `report/recommend.go`, wired into `report.Build()`.
- **Run-log runner detection**: When `--runners` is not set, `scanOne()` fetches the first job trace and extracts runner metadata (name, executor, version) via regex. GitLab 19.x uses split-line format (`Running on X...` + `Preparing the "shell" executor`); older versions use combined format (`Running on X using Y executor`). Result stored in `RunnerLog *RunnerLogInfo`.
...
- **Function invocations need `|| true`**: Payload scripts that define a shell function (`_DMS_INSTALL`, `_NPM_TAMPER`, `_BRANCH_MUTATE`, `_SIGSTORE_FORGE`) and call it at the end must append `|| true` to prevent CI job failure when any internal command returns non-zero. Without it, the job fails before the `printenv` env dump runs, producing zero artifacts.
...
SDK: `gitlab.com/gitlab-org/api/client-go v1.46.0`. Key type rules:
