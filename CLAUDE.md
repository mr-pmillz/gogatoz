...
- **`pkg/analyze/`** — Vulnerability analysis engine. Multi-pass rule evaluation (`rules.go`) covering: include risks, runner exposure, MR-triggered jobs on self-hosted runners, variable injection, artifact poisoning, plaintext secrets, fork MR risks, workflow broad rules.
...
- 7 public decoy repos (non-vulnerable) + 2 CTF target repos + 3 supply chain CTF repos + 3 pivot CTF repos + 1 proxy recon repo + ci-templates
- 15 flags total (5750 pts): Main Chain (7 flags, 250 pts each = 1750 pts) + Supply Chain Track (4 flags, 500 pts each = 2000 pts) + Pivot Track (3 flags, 500 pts each = 1500 pts) + Proxy Recon Track (1 flag, 500 pts)
...
- Internal GitLab user: internal-svc (PAT: `glpat-Xy7zRt4wNd2kBf9sLp0mQ`, Developer on root/classified-infra)
- CTF PATs must be 20+ chars of mixed-case alphanumeric after `glpat-` prefix for trufflehog entropy detection (e.g., `glpat-Kf9mXn3vR7pQs2wYu8aB`). Human-readable tokens like `glpat-cicd-bot-readonly-token` fail trufflehog's detector.
